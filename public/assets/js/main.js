...
// $(".cart-plus-minus").append('<div class="inc qtybutton">+</div>');
        ...
        $(".inc,.dec").on("click", function() {
            ...
            var finalPrice = $(this).parent().parent().next().next();

            var tr = $(this).parent().parent().parent();

            var checkoutTd = $("#checkoutprice");

            $.ajax({
                url:"http://localhost/store_laravel/public/cart/counter",
                type:"post",
                data:{id:dataId,_token:csrf,dir:direction},
                success:function(receivedData){

                    if(receivedData.status == "error"){
                        alert(receivedData.msg);
                    }else if(receivedData.status = "success"){

                        if(receivedData.newCount == 0){
                            tr.hide(500,function(){
                                $(this).remove();
                            });
                        }else{
                            input.val(receivedData.newCount);

                            var totalPrice = receivedData.price*receivedData.newCount;

                            var priceWithOffer = totalPrice-(totalPrice/100*receivedData.offer);

                            finalPrice.text(priceWithOffer+" تومان");

                            checkoutTd.text(receivedData.checkout+" تومان");
                        }
                    }
